/* assets/js/services/tours-mobile-vehicle-gallery.js
   Tours mobile vehicle gallery.
   Responsabilidad:
   - cargar catálogo de fotos de flota desde JSON
   - abrir un lightbox/gallery viewer desde la foto del footer móvil Tours
   - preparado para varias fotos
   - no tocar tours-mobile-config-step.js
   - no tocar tours-panel.js
   - no tocar desktop
   - no tocar tarifa, CTA ni payload
*/

(function initToursMobileVehicleGallery(window, document) {
  "use strict";
  ...
  var BODY_SCREEN_ATTR = "data-tours-mobile-config-screen";
  var BODY_GALLERY_ATTR = "data-tours-mobile-vehicle-gallery-active";

  var TRIGGER_SELECTOR = ".tours-mobile-config-step__vehicle";
  var TRIGGER_IMAGE_SELECTOR = ".tours-mobile-config-step__vehicle-image";

  var GALLERY_SELECTOR = "[data-tours-mobile-vehicle-gallery]";
  var IMAGE_SELECTOR = "[data-tours-mobile-vehicle-gallery-image]";
  var COUNTER_SELECTOR = "[data-tours-mobile-vehicle-gallery-counter]";
  var LABEL_SELECTOR = "[data-tours-mobile-vehicle-gallery-label]";
  var CLOSE_SELECTOR = "[data-tours-mobile-vehicle-gallery-close]";
  var PREV_SELECTOR = "[data-tours-mobile-vehicle-gallery-prev]";
  var NEXT_SELECTOR = "[data-tours-mobile-vehicle-gallery-next]";
  var BACKDROP_SELECTOR = "[data-tours-mobile-vehicle-gallery-backdrop]";
  ...
  function isToursMobileConfigActive() {
    return (
      isMobileViewport() &&
  ...
  function syncTrigger(vehicle) {
    ...
    trigger.setAttribute("data-tours-mobile-vehicle-gallery-trigger", "1");
  ...
  function buildGalleryNode() {
    ...
    root.className = "tours-mobile-vehicle-gallery";
    root.setAttribute("data-tours-mobile-vehicle-gallery", "1");
    ...
    backdrop.className = "tours-mobile-vehicle-gallery__backdrop";
    backdrop.setAttribute("data-tours-mobile-vehicle-gallery-backdrop", "1");

    panel.className = "tours-mobile-vehicle-gallery__panel";
    panel.setAttribute("role", "document");

    close.type = "button";
    close.className = "tours-mobile-vehicle-gallery__close";
    close.setAttribute("data-tours-mobile-vehicle-gallery-close", "1");
    close.textContent = "×";

    imageWrap.className = "tours-mobile-vehicle-gallery__image-wrap";

    image.className = "tours-mobile-vehicle-gallery__image";
    image.setAttribute("data-tours-mobile-vehicle-gallery-image", "1");
    image.decoding = "async";

    footer.className = "tours-mobile-vehicle-gallery__footer";

    label.className = "tours-mobile-vehicle-gallery__label";
    label.setAttribute("data-tours-mobile-vehicle-gallery-label", "1");

    counter.className = "tours-mobile-vehicle-gallery__counter";
    counter.setAttribute("data-tours-mobile-vehicle-gallery-counter", "1");

    prev.type = "button";
    prev.className = "tours-mobile-vehicle-gallery__nav tours-mobile-vehicle-gallery__nav--prev";
    prev.setAttribute("data-tours-mobile-vehicle-gallery-prev", "1");
    prev.textContent = "‹";

    next.type = "button";
    next.className = "tours-mobile-vehicle-gallery__nav tours-mobile-vehicle-gallery__nav--next";
    next.setAttribute("data-tours-mobile-vehicle-gallery-next", "1");
    next.textContent = "›";
  ...
  function openGallery(index) {
    ...
    if (!isToursMobileConfigActive() || !vehicle || !gallery) {
      return false;
    }
  ...
  function bindGalleryEvents() {
    if (!galleryNode || galleryNode.dataset.toursMobileVehicleGalleryBound === "1") {
      return false;
    }

    galleryNode.dataset.toursMobileVehicleGalleryBound = "1";
  ...
  function bindDocumentEvents() {
    document.addEventListener("click", function onTriggerClick(event) {
      ...
      if (!trigger || !isToursMobileConfigActive()) {
        return;
      }
  ...
  window.PixkuyToursMobileVehicleGallery = {
    open: openGallery,
    close: closeGallery,
    next: function next() {
      return goToSlide(1);
