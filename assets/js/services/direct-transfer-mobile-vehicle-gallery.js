/* assets/js/services/direct-transfer-mobile-vehicle-gallery.js
   Direct Transfer mobile vehicle gallery.
   Responsabilidad:
   - cargar catálogo de fotos de flota desde JSON
   - abrir lightbox/gallery viewer desde la foto del footer móvil Direct Transfer
...
(function initDirectTransferMobileVehicleGallery(window, document) {
  "use strict";
  ...
  var BODY_SCREEN_ATTR = "data-direct-transfer-mobile-config-screen";
  var BODY_GALLERY_ATTR = "data-direct-transfer-mobile-vehicle-gallery-active";

  var TRIGGER_SELECTOR = ".direct-transfer-mobile-config-step__vehicle";
  var TRIGGER_IMAGE_SELECTOR = ".direct-transfer-mobile-config-step__vehicle-image";

  var GALLERY_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery]";
  var IMAGE_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-image]";
  var COUNTER_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-counter]";
  var LABEL_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-label]";
  var CLOSE_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-close]";
  var PREV_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-prev]";
  var NEXT_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-next]";
  var BACKDROP_SELECTOR = "[data-direct-transfer-mobile-vehicle-gallery-backdrop]";
  ...
  function isDirectTransferMobileConfigActive() {
    return (
      isMobileViewport() &&
  ...
  function syncTrigger(vehicle) {
    ...
    trigger.setAttribute("data-direct-transfer-mobile-vehicle-gallery-trigger", "1");
  ...
  function buildGalleryNode() {
    ...
    root.className = "direct-transfer-mobile-vehicle-gallery";
    root.setAttribute("data-direct-transfer-mobile-vehicle-gallery", "1");
    ...
    backdrop.className = "direct-transfer-mobile-vehicle-gallery__backdrop";
    backdrop.setAttribute("data-direct-transfer-mobile-vehicle-gallery-backdrop", "1");

    panel.className = "direct-transfer-mobile-vehicle-gallery__panel";
    panel.setAttribute("role", "document");

    close.type = "button";
    close.className = "direct-transfer-mobile-vehicle-gallery__close";
    close.setAttribute("data-direct-transfer-mobile-vehicle-gallery-close", "1");
    close.textContent = "×";

    imageWrap.className = "direct-transfer-mobile-vehicle-gallery__image-wrap";

    image.className = "direct-transfer-mobile-vehicle-gallery__image";
    image.setAttribute("data-direct-transfer-mobile-vehicle-gallery-image", "1");
    image.decoding = "async";

    footer.className = "direct-transfer-mobile-vehicle-gallery__footer";

    label.className = "direct-transfer-mobile-vehicle-gallery__label";
    label.setAttribute("data-direct-transfer-mobile-vehicle-gallery-label", "1");

    counter.className = "direct-transfer-mobile-vehicle-gallery__counter";
    counter.setAttribute("data-direct-transfer-mobile-vehicle-gallery-counter", "1");

    prev.type = "button";
    prev.className = "direct-transfer-mobile-vehicle-gallery__nav direct-transfer-mobile-vehicle-gallery__nav--prev";
    prev.setAttribute("data-direct-transfer-mobile-vehicle-gallery-prev", "1");
    prev.textContent = "‹";

    next.type = "button";
    next.className = "direct-transfer-mobile-vehicle-gallery__nav direct-transfer-mobile-vehicle-gallery__nav--next";
    next.setAttribute("data-direct-transfer-mobile-vehicle-gallery-next", "1");
    next.textContent = "›";
  ...
  function openGallery(index) {
    ...
    if (!isDirectTransferMobileConfigActive() || !vehicle || !gallery) {
      return false;
    }
  ...
  function bindGalleryEvents() {
    if (!galleryNode || galleryNode.dataset.directTransferMobileVehicleGalleryBound === "1") {
      return false;
    }

    galleryNode.dataset.directTransferMobileVehicleGalleryBound = "1";
  ...
  function bindDocumentEvents() {
    document.addEventListener("click", function onTriggerClick(event) {
      ...
      if (!trigger || !isDirectTransferMobileConfigActive()) {
        return;
      }
  ...
  function syncActiveState() {
    ...
    if (!isDirectTransferMobileConfigActive()) {
      if (isOpen()) {
        closeGallery();
      }
  ...
  window.PixkuyDirectTransferMobileVehicleGallery = {
    open: openGallery,
    close: closeGallery,
    next: function next() {
      return goToSlide(1);
